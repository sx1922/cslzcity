if CLIENT then
    ...
    net.Receive("ZB_NotifyRoundListChange", function()
        ...
        chat.AddText(Color(180, 180, 255), playerName, COL_TEXT, " 修改了游戏模式队列")
    ...
    local function CreateAvailableRow(parent, mode, manager)
        ...
        local statusText, statusCol
            if mode.canlaunch == 1 then
                statusText, statusCol = "可启动", COL_GREEN_H
            elseif mode.canlaunch == 0 then
                statusText, statusCol = "无法启动（无点数 / 被禁用）", COL_ACCENT_H
            else
    ...
    local function OpenQueueManager()
        ...
        header.Paint = function(self, w, h)
            ...
            surface.SetDrawColor(COL_CATBAR)
            surface.DrawRect(0, h - 5, w, 5)
            draw.SimpleText("游戏模式队列", "ZB_QM_Title", w / 2, h / 2 - 2, COL_TEXT, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
        ...
        leftPanel.Paint = nil
        CreateCategoryBar(leftPanel, "可用游戏模式")
        ...
        searchBar.Paint = function(self, w, h)
            ...
            if self:GetText() == "" then
                draw.SimpleText("搜索游戏模式...", "ZB_QM_Item", 8, h / 2, COL_TEXT_DIM, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            end
        end
        ...
        CreateCategoryBar(queuePanel, "回合队列")
        ...
        applyBtn.DoClick = function()
            net.Start("ZB_UpdateRoundList")
                ...
                net.WriteBool(true)
            net.SendToServer()
            chat.AddText(COL_GREEN_H, "游戏模式队列已设置！")
            surface.PlaySound(SND_CLICK)
        end
        ...
        clearBtn.DoClick = function()
            ...
            frame:QueueUpdate()
            surface.PlaySound(SND_CLICK)
            chat.AddText(COL_ORANGE, "队列已清空（按“应用”保存）")
        end
        ...
        queueList.Paint = function(self, w, h)
            if #self.rows == 0 then
                draw.SimpleText("队列为空，模式将随机选择。", "ZB_QM_Item", 6, 12, COL_TEXT_DIM, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
            end
        end
        ...
        local function MakeQueueRow(modeKey)
            ...
            row.Paint = function(self, w, h)
                ...
                local label = isNext and (forced and "强制" or "下一个") or ("#" .. idx)
    ...
    local function OpenAdminMenu()
        ...
        header.Paint = function(self, w, h)
            ...
            surface.SetDrawColor(COL_CATBAR)
            surface.DrawRect(0, h - 5, w, 5)
            draw.SimpleText("管理面板", "ZB_QM_Title", w / 2, h / 2 - 2, COL_TEXT, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
        AddCloseButton(header, frame)
        ...
        local manageBtn = BigButton("管理游戏模式队列", COL_ROW, COL_ROW_HOV)
        manageBtn.DoClick = function()
            surface.PlaySound(SND_CLICK)
            OpenQueueManager()
        end

        local endBtn = BigButton("结束回合", COL_ACCENT, COL_ACCENT_H)
