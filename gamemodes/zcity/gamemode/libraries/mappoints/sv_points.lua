...
function zb.GetMapPoints( pointGroup, forceupdatepoints ) -- Загрузить точки в память игры... На клиенте будет примерно такая же функция.
    if not zb.CreateMapDir() then PrintMessage( HUD_PRINTTALK, "sv_points.lua: 地图文件夹不存在？" ) return false end
    if not zb.Points[pointGroup] then PrintMessage( HUD_PRINTTALK, "sv_points.lua: 点组 " .. "\"" .. pointGroup .. "\"" .. " 不存在。" ) return false end
...
function zb.RemoveMapPoint( pointGroup, pointNum, needsave, removeall ) -- Создать точку на карте, и сохранить ли ее?
    ...
    if removeall then zb.Points[pointGroup].Points = {} else
        if not zb.Points[pointGroup].Points[ math.Clamp(pointNum or 0, 1, #zb.Points[pointGroup].Points) ] then PrintMessage( HUD_PRINTTALK, "sv_points.lua: 点不存在。" ) return false end
...
function zb.SetMapPoint( pointGroup, pointNum, pointData, needsave ) -- Создать точку на карте, и сохранить ли ее?
    if not zb.CreateMapDir() then PrintMessage( HUD_PRINTTALK, "sv_points.lua: 无法创建地图文件夹。" ) return false end
    if not zb.Points[pointGroup] then PrintMessage( HUD_PRINTTALK, "sv_points.lua: 点组 " .. "\"" .. pointGroup .. "\"" .. " 不存在。" ) return false end

    zb.Points[pointGroup].Points = zb.Points[pointGroup].Points or zb.GetMapPoints( pointGroup )
    if not zb.Points[pointGroup].Points[ math.Clamp(pointNum, 1, #zb.Points[pointGroup].Points) ] then PrintMessage( HUD_PRINTTALK, "sv_points.lua: 点不存在。" ) return false end
...
COMMANDS.pointnew = {function(ply,args)
    if not args[1] then
        ply:ChatPrint("用法：!pointnew <pointGroup>")
        return
    end
...
COMMANDS.pointset = {function(ply,args)
    if not args[1] or not args[2] then
        ply:ChatPrint("用法: !pointset <点组名> <点编号>")
        return
    end
...
COMMANDS.pointremove = {function(ply,args)
    if not args[1] then
        ply:ChatPrint("用法：!pointremove <pointGroup> <pointNumber|*>\n使用 * 可移除所有点")
        return
    end
...
net.Receive("zb_getallpoints",function(len,ply)
    if not ply:IsAdmin() then ply:ChatPrint("点：访问被拒绝") return end
