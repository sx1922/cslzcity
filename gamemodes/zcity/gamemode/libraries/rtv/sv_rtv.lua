...
COMMANDS.forcertv = {function(ply, args)
	if not ply:IsAdmin() then ply:ChatPrint("您没有权限") return end
		zb.StartRTV(20)
	end,
	0
}
...
local function rtv(ply, args)
    ...
    for _, v in player.Iterator() do
        if remaining != 0 then
            v:ChatPrint(
                ply:Nick() .. " 投票换图。 " .. 
                remaining .. " 票还需投票。输入 !rtv 再次取消您的投票。"
            )
        end
    end
