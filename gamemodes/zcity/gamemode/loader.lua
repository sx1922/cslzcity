...
local function InitMode()
	...
	if not isstring(name) or name == "" then
		print("[Z-City] skipped mode without a name")
		return
	end
	local saved = zb.modes[name] and zb.modes[name].saved or {} -- saved table is used for saving data between hotloads

	if MODE.base then
		local baseMode = zb.modes[MODE.base]
		if not baseMode then
			ErrorNoHalt("[Z-City] mode '" .. name .. "' requested missing base '" .. tostring(MODE.base) .. "'\n")
			return
		end
		table.Inherit(MODE, baseMode)

		for i, tbl in pairs(MODE) do
			if istable(MODE[i]) and istable(baseMode[i]) then
				local tbl2 = {}
