...
function MODE:HUDPaint()
    ...
    draw.SimpleText("超级战士3D", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local Rolename = fighter.name
	local ColorRole = fighter.color1
    ColorRole.a = 255 * fade
    draw.SimpleText("你是一名"..Rolename , "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.5, ColorRole, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
CreateEndMenu = function()
    ...
    hmcdEndMenu.Paint = function(self,w,h)
		BlurBackground(self)
		local txt = (IsValid(wonply) and wonply:GetPlayerName() or "无人").." 获胜！"
	...
	for i,ply in player.Iterator() do
		...
		but.Paint = function(self,w,h)
			...
			surface.DrawText((ply:Name() .. (not ply:Alive() and " - 已阵亡" or "")) or "已退出...")
		...
		function but:DoClick()
			...
			if ply:IsBot() then chat.AddText(Color(255,0,0), "不行，这做不到") return end
