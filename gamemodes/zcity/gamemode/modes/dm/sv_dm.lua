...
function MODE:RoundStart()
	...
	for _, ply in player.Iterator() do
		...
		zb.GiveRole(ply, "战士", Color(190,15,15))
...
function MODE:GiveEquipment()
	for _, ply in player.Iterator() do
		if !IsValid(ply) or !ply:Alive() or ply:Team() == TEAM_SPECTATOR then continue end
		GiveWeaponToPlayer(ply)
	end
end

-- 玩家（重生/中途加入）时配给武器，避免出生无枪
hook.Add("PlayerSpawn", "dm_giveweapons", function(ply)
	local rnd = CurrentRound()
	if not rnd or rnd.name != "dm" then return end
