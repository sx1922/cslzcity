...
function MODE:Intermission()
	...
	for k, ply in player.Iterator() do
		...
		if ply.organism then ply.organism.recoilmul = DefaultSkillIssue end
			...
			if v.GetPhysicsObject and IsValid(v:GetPhysicsObject()) and !self:IsDoor(v) then
					if math.random(1, 50) == 1 then
						...
						if !IsValid(ply.FakeRagdoll) then
							...
							local matrix
							if isnumber(boneId) and boneId >= 0 then
								local ok, value = pcall(ply.GetBoneMatrix, ply, boneId)
								if ok then matrix = value end
							end
							pos = matrix and matrix:GetTranslation() or ply:WorldSpaceCenter()
						elseif IsValid(ply.FakeRagdoll) then
							local boneId = ply.FakeRagdoll:LookupBone("ValveBiped.Bip01_Head1")
							local matrix
							if isnumber(boneId) and boneId >= 0 then
								local ok, value = pcall(ply.FakeRagdoll.GetBoneMatrix, ply.FakeRagdoll, boneId)
								if ok then matrix = value end
							end
							pos = matrix and matrix:GetTranslation() or ply.FakeRagdoll:WorldSpaceCenter()
					end
