...
function MODE:HUDPaint()
    ...
    local Rolename = isEventer and "活动员" or GetGlobalString("ZB_EventRole","玩家")
...
CreateEndMenu = function()
	...
	for i, ply in player.Iterator() do
		...
		but.Paint = function(self,w,h)
			...
			surface.SetFont( "ZB_InterfaceMediumLarge" )
			local lengthX, lengthY = surface.GetTextSize( ply:GetPlayerName() or "已离开游戏..." )
			
			surface.SetTextColor(0,0,0,255)
			surface.SetTextPos(w / 2 + 1,h/2 - lengthY/2 + 1)
			surface.DrawText(ply:GetPlayerName() or "已离开游戏...")
			...
			surface.DrawText((ply:Name() .. (not ply:Alive() and " - 已死亡" or "")) or "已离开游戏...")

			surface.SetFont( "ZB_InterfaceMediumLarge" )
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lengthX, lengthY = surface.GetTextSize( ply:Frags() or "已离开游戏..." )
			surface.SetTextPos(w - lengthX -15,h/2 - lengthY/2)
			surface.DrawText(ply:Frags() or "已离开游戏...")
		end

		function but:DoClick()
			if ply:IsBot() then chat.AddText(Color(255,0,0), "不行，你无法查看") return end
...
local function CreateLootPollingMenu()
    ...
    LootPollingMenu:SetTitle("事件战利品管理器")
    ...
    classEntry:SetPlaceholderText("weapon_name 或 prop_physics")
    ...
    local removeButton = createButton(buttonPanel, 15, 20, 140, 30, "移除所选", 
    ...
    local resetButton = createButton(buttonPanel, 505, 20, 140, 30, "全部重置", 
        Color(180, 10, 10), Color(220, 30, 30),
        function()
    ...
    local specialButton = createButton(buttonPanel, 165, 20, 160, 30, "从列表选择", 
        ...
        function()
            ...
            local meleeSubMenu = menu:AddSubMenu("近战武器")
            ...
            explosiveSubMenu:AddOption("火箭筒", function() classEntry:SetValue("weapon_hg_rpg") end)
    ...
    local healButton = createButton(buttonPanel, 335, 20, 160, 30, "医疗物品", 
