...
concommand.Add("tdm_setrounds", function(ply, cmd, args)
    ...
    PrintMessage(HUD_PRINTTALK, "TDM 回合数已设为 "..MODE.Rounds.."。剩余回合："..zb.RoundsLeft)
end)
...
COMMANDS.nextcsround = {
	function(ply, args)
		...
		if string.lower(args[1]) == "bomb" then
            zb.nextcsround = "bomb"
            PrintMessage(HUD_PRINTTALK, "本局 CS 模式 - 炸弹")
        end

        if string.lower(args[1]) == "hostage" then
            zb.nextcsround = "hostage"
            PrintMessage(HUD_PRINTTALK, "本局 CS 模式 - 人质")
        end
	end,
	0
}
...
function MODE:EndRound()
    ...
    elseif zb.rtype == "hostage" then
        ...
        if IsValid(zb.hostage) and not zb.hostage.organism.alive then
            ...
            if zb.HarmDoneDetailed[zb.hostage:EntIndex()] then
                ...
                PrintMessage(HUD_PRINTTALK, (maxTeam == 0 and "恐怖分子" or "反恐精英") .. " 杀害了人质")
            else
                winner = 3
            end
        end
    ...
    local winnerprt = (winner == 1 and "反恐精英") or (winner == 0 and "恐怖分子") or "没有人"
	...
	if winsTeam0 > winsTeam1 then
		for _, ply in ipairs(team.GetPlayers(1)) do
			...
			ply:ChatPrint("你的团队正在失利，你获得了1000金钱补偿。")
		end
