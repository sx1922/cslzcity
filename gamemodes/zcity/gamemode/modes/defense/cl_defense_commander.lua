...
function CreateCommanderMenu()
    ...
    commanderMenu.Paint = function(self, w, h)
        ...
        draw.SimpleText("指挥官补给请求", "CommanderTitle", w/2, 25, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

        draw.SimpleText("可用点数：" .. points, "CommanderText", w - 120, 25, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
    end
    ...
    cartPanel.Paint = function(self, w, h)
        ...
        draw.SimpleText("你的订单", "CommanderCategory", 10, 10, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_LEFT)
        
        local totalCost = CalculateCartCost()
        draw.SimpleText("总花费：" .. totalCost .. " 点数", "CommanderText", w - 10, 10, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_RIGHT)
    end
    ...
    local function UpdateCartDisplay()
        ...
        for i, item in ipairs(currentCart) do
            ...
            cartItem.Paint = function(self, w, h)
                ...
                draw.SimpleText(item.price .. " 点 x " .. (item.quantity or 1), "CommanderSmall", w/2 + 10, 35, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            end
    ...
    orderButton.Paint = function(self, w, h)
        ...
        draw.SimpleText("下达订单", "CommanderText", w/2, h/2, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    orderButton.DoClick = function()
        ...
        if totalCost > points then
            chat.AddText(COMMANDER_UI.PRIMARY_COLOR, "补给点数不足！")
    ...
    local function PopulateItems(category)
        ...
        for _, item in ipairs(availableItems[category]) do
            ...
            itemButton.Paint = function(self, w, h)
                ...
                draw.SimpleText(item.price .. " 点数", "CommanderText", w - 150, 20, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_RIGHT)
            ...
            addToCartBtn.DoClick = function()
                if item.price * quantity > points then
                    chat.AddText(COMMANDER_UI.PRIMARY_COLOR, "该物品的补给点数不足！")
                    surface.PlaySound("buttons/button10.wav")
                    return
                end
...
local COMMANDER_HINT = {
    ...
    text = [[作为指挥官，你负责支援你的团队！

使用 Q 菜单为你的士兵订购装备和支援。

每波攻击结束后你会获得补给点数，请明智使用！
