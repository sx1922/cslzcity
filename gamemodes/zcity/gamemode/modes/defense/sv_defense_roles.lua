...
function MODE:AddCommanderPoints(points)
    for _, ply in player.Iterator() do
        if ply:GetNWString("PlayerRole") == "指挥官" and ply:Alive() then
            ...
            net.WriteString("你获得了 " .. points .. " 补给点！")
...
function MODE:AssignPlayerRoles()
    ...
    if commander then
        commander:SetNWString("PlayerRole", "指挥官")
    ...
    for i = 1, numMedics do
        ...
        ply:SetNWString("PlayerRole", "医疗兵")
        EquipBaseGear(ply)
    ...
    for i = 1, numEngineers do
        local ply = table.remove(players, math.random(#players))
        if not ply or not IsValid(ply) then continue end
        ply:SetNWString("PlayerRole", "工程师")
        EquipBaseGear(ply)
...
function MODE:GiveEquipment()
    ...
    for _, ply in player.Iterator() do
        ...
        pcall(function()
            ...
            zb.GiveRole(ply, "难民", Color(255, 150, 0))
        end)
...
function SetCommanderRoleByID(playerID)
    ...
    if IsValid(ply) then
        ply:SetNWString("PlayerRole", "指挥官")
