...
function MODE:HUDPaint()
	if NextWave_Time > CurTime() - 5 then
		...
		draw.SimpleText( "下一波进攻将在 ".. time.m ..":" .. time.s, "ZB_HomicideMedium", sw * 0.5, sh * (0.9 + timePos), Color(87,146,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
    ...
    draw.SimpleText("Z战斗 | HL2 基地防御", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	if hg.PluvTown.Active then
		...
		draw.SimpleText("普拉夫镇的某个地方", "ZB_ScrappersLarge", sw / 2, sh * 0.44 - ScreenScale(2), Color(0, 0, 0, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end
...
CreateEndMenu = function()
	...
	for i,ply in player.Iterator() do
		...
		but.Paint = function(self,w,h)
			...
			surface.SetFont( "ZB_InterfaceMediumLarge" )
			local lengthX, lengthY = surface.GetTextSize( ply:GetPlayerName() or "他退出了..." )
			
			surface.SetTextColor(0,0,0,255)
			surface.SetTextPos(w / 2 + 1,h/2 - lengthY/2 + 1)
			surface.DrawText(ply:GetPlayerName() or "他退出了...")
			...
			surface.DrawText((ply:Name() .. (not ply:Alive() and " - 已死亡" or "")) or "他退出了...")

			surface.SetFont( "ZB_InterfaceMediumLarge" )
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lengthX, lengthY = surface.GetTextSize( ply:Frags() or "他退出了..." )
			surface.SetTextPos(w - lengthX -15,h/2 - lengthY/2)
			surface.DrawText(ply:Frags() or "他退出了...")
		end

		function but:DoClick()
			if ply:IsBot() then chat.AddText(Color(255,0,0), "不行，你不能") return end
...
local function CreateVoteMenu()
    ...
    voteMenu.Paint = function(self, w, h)
        ...
        draw.SimpleText("投票选择本回合的模式", "Defense_Subtitle", w / 2, 55, Color(200, 200, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    local modeDescriptions = {
        ...
        [2] = {
            title = "扩展模式",
            shortDesc = "12 波，含首领和特殊敌人",
            longDesc = "老玩家的挑战！12 波无情的血腥与残酷。",
    ...
    local function CreateModeButton(index, yPos)
        ...
        descPanel.Paint = function(self, w, h)
            ...
            if isDisabled then
                draw.SimpleText("该模式目前正在开发中，即将推出！", 
...
local function DrawBossIncomingBanner()
    ...
    draw.SimpleText("首领来袭", "ZB_HomicideMediumLarge", centerX + 2, textY + 2, shadowColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    

    draw.SimpleText("首领来袭", "ZB_HomicideMediumLarge", centerX, textY, textColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
