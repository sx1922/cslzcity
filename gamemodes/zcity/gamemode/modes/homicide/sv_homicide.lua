...
MODE.Types.standard = {
	...
	PoliceEquipment = function(ply)
		...
		zb.GiveRole(ply, "警察", Color(15,15,255))
	end
}
MODE.Types.wildwest = {
	Chance = 0.05,
...
MODE.Types.soe = {
	...
	PoliceEquipment = function(ply)
		...
		zb.GiveRole(ply, "国民警卫队", Color(55, 85, 0))
		ply:SetNetVar("CurPluv", "pluvberet")
	end,
	PoliceText = "国民警卫队已经抵达。",
...
function MODE:CheckAlivePlayers()
	...
	for _, ply in player.Iterator() do
		...
		-- 跳过已倒地/被击杀（otrub）的玩家。
		-- 注意：incapacitated 仅在 otrub 且满足额外致命条件时才为 true，
		-- 仅检查 incapacitated 会让部分"刚被击杀、尚未真正 Kill"的杀手
		-- 仍被计入存活阵营，导致对局无法结束。
		if ply.organism and (ply.organism.incapacitated or ply.organism.otrub) then
			continue
		end

		if ply.isTraitor then
			-- 被手铐铐住的杀手已被制服，不应冒充无辜者阵营，也不计入杀手阵营
...
function MODE:RoundThink()
	...
	if self.Type ~= "soe" and not self.swatDeployed and self.deadPoliceCount >= (self.spawnedPoliceCount or 4) and self.spawnedPoliceCount > 0 then
		...
		timer.Create("HMCDSpawnSWAT", 60, 1, function()
			...
			if count > 0 then
				PrintMessage(HUD_PRINTTALK, "SWAT小队即将到达！")
	...
	if self.Type == "soe" and not self.PoliceSpawned and self.saved.PoliceTime < CurTime() then
		...
		if count > 0 then
			...
			if spawned > 0 then
				self.PoliceSpawned = true
				PrintMessage(HUD_PRINTTALK, self.Types[self.Type].PoliceText or "国民警卫队已经抵达。")
...
function MODE:EquipSWAT(ply, index)
    ...
    zb.GiveRole(ply, "SWAT 特勤队员", Color(30, 30, 100))
end
...
function MODE:EquipNationalGuard(ply, index)
    ...
    local hands = ply:Give("weapon_hands_sh")
    ply:SetActiveWeapon(hands)
    zb.GiveRole(ply, "国民警卫队", Color(60, 90, 0))
end
...
function MODE:RoundStart()
	...
	if(roles_choose)then
		...
		PrintMessage(HUD_PRINTTALK, "叛徒正在挑选角色，" .. MODE.RoleChooseRoundStartTime ..  " 秒")
	else
		MODE.ChoosingPlayersList = {}
...
function MODE:EndRound()
	...
	if self.Type then
		if(MODE.RoleChooseRound)then
			if(winner ~= 1)then
				PrintMessage(HUD_PRINTTALK, "所有叛徒都已被制止。")
				...
				PrintMessage(HUD_PRINTTALK, "所有无辜者都被杀害了。")
			end
			
			timer.Simple(2, function()
				if(players_alive == 0)then
					PrintMessage(HUD_PRINTTALK, "无人生还。")
				else
					if(players_alive == 1)then
						PrintMessage(HUD_PRINTTALK, "城市中只剩下 1 名幸存者。")
					else
						PrintMessage(HUD_PRINTTALK, "城市中还剩 " .. players_alive .. " 名幸存者。")
					end
				end
			end)
		else
