...
function MODE:HUDPaint()
	...
	if startTimer > CurTime() then
		...
		local w, h = surface.GetTextSize("等待玩家：")
    ...
    draw.SimpleText("Homicide | 合作模式", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local Rolename = (lply.role and lply.role.name) or "未知"
    ...
    local Objective = lply.PlayerClassName == "Gordon" and "带领反抗军取得胜利！" or "跟随戈登！"
...
CreateEndMenu = function()
	...
	for i, ply in player.Iterator() do
		...
		but.Paint = function(self,w,h)
			...
			surface.SetFont( "ZB_InterfaceMediumLarge" )
			local lengthX, lengthY = surface.GetTextSize( ply:GetPlayerName() or "已退出..." )
			
			surface.SetTextColor(0,0,0,255)
			surface.SetTextPos(w / 2 + 1,h/2 - lengthY/2 + 1)
			surface.DrawText(ply:GetPlayerName() or "已退出...")
			...
			surface.DrawText((ply:Name() .. (not ply:Alive() and " - 死亡" or "")) or "已退出...")

			surface.SetFont( "ZB_InterfaceMediumLarge" )
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lengthX, lengthY = surface.GetTextSize( ply:Frags() or "已退出..." )
			surface.SetTextPos(w - lengthX -15,h/2 - lengthY/2)
			surface.DrawText(ply:Frags() or "已退出...")
		end

		function but:DoClick()
			if ply:IsBot() then chat.AddText(Color(255,0,0), "不，你不能") return end
