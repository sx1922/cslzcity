...
if CLIENT then
	...
	concommand.Add("vfire_default_visual_settings", function()
		...
		vFireMessage("vFire客户端设置已重置为默认值！")
	end)
end
...
if CLIENT then
	if GetConVar("gmod_mcore_test"):GetInt() == 0 then
		vFireMessage("vFire在gmod_mcore_test设置为1时性能最佳，请启用该设置并重启游戏以使更改生效。")
	end
end
...
---------------------------------------------------------------------------]]
	...
	if SERVER or CLIENT then
		hook.Add("InitPostEntity", "vFireULXSupport", function()
			...
			function ulx.vextinguish(ply)
				...
				ulx.fancyLogAdmin(ply, "#A 扑灭了 "..removeCount.." 处火焰。")
			end
			...
			function ulx.vextinguishall(ply)
				...
				ulx.fancyLogAdmin(ply, "#A 扑灭了全部 "..removeCount.." 处火焰。")
			end
			...
			function ulx.vstartfire(ply, size)
				...
				ulx.fancyLogAdmin(ply, "#A 点燃了一处火焰。")
			end
	...
	if SERVER then
		hook.Add("vFire - StormFox Handeshake", "vFire - StormFox Handeshake", function()
			vFireMessage("我们每晚都做着同样的事，StormFox。尝试征服世界！>:D")
		end)
	end