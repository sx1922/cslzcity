...
module[2] = function(owner, org, timeValue)
	...
	if org.isPly and not org.otrub and o2.curregen < losing_oxy and org.analgesia <= 1.5 and !org.heartstop then
		if mask_blevota then
			if o2[1] < 15 then
				org.owner:Notify("摘掉这个该死的面具", 25, "take_gasmask2", 0, nil, color_red2)
			else
	...
	if org.isPly then
		if org.pneumothorax > 0 then
			org.owner:Notify("我能感觉到有什么东西充满了我的肺部。", true, "pneumothorax1",10) // delay of 10 seconds before typing that
		else
			org.owner:ResetNotification("pneumothorax1")
		end

		if org.pneumothorax > 0.3 then
			org.owner:Notify("呼吸变得越来越困难。", true, "pneumothorax2", 5)
		else
			org.owner:ResetNotification("pneumothorax2")
		end

		if org.pneumothorax > 0.5 then
			org.owner:Notify("我真的很难呼吸。", true, "pneumothorax3", 5)
		else
	...
	if org.isPly then
		if org.brain > 0.1 and org.brain < 0.3 then
			org.owner:Notify(math.random(2) == 1 and "我的头好痛……" or "我在哪里？", true, "brain", 5)
		else
