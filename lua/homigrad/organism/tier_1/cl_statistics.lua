...
hook.Add("HUDPaint","homigrad-wound-debug",function()
	if not lply:Alive() then
		...
		if timeHuy and ((timeHuy + traveltime) > CurTime()) then
			...
			draw.SimpleText("按R跳过.", "HomigradFontBig", ScrW() / 3 * 2, ScrH() / 7, color_white)
			draw.SimpleText("命中 "..tostring(iter).." / "..tostring(#hg.hits).." 由 "..inf.." 来自 "..attacker, "HomigradFontBig", ScrW() / 3 * 2, ScrH() / 10, color_white)
