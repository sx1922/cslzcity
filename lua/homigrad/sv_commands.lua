...
if SERVER then
    ...
    COMMANDS.zc_god = {function(ply)
		...
		ply:Notify(ply.organism.godmode and "我现在刀枪不入……" or "我现在只是凡人了")
		return
    end,1}
	...
	COMMANDS.zc_cloak = {function(ply)
		...
		ply:RemoveAllDecals()
		ply:Notify(ply.cloak and "我现在隐形了……" or "我现在可见了") -- walking by the wall
		return
    end,1}

    COMMANDS.punish = {function(ply, args)
        if #args < 1 then
            ply:ChatPrint("把这个 OwO 的名字告诉我吧。")
            return
        end
        ...
        ply:ChatPrint("肥宅 " .. target:Nick() .. " 已被惩罚。")
    end, 2, "玩家昵称"}
    ...
    COMMANDS.notify = {function(ply, args)
        ...
        ply:ChatPrint("已向 " .. target:GetName() .. " 发送通知：" .. message)
