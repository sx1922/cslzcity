...
function hg.AddArmor(ply, equipment, ent)
    ...
    -- Mode loadouts commonly pass several armor pieces at once. Expand the
    -- list before string-based validation; otherwise string.Replace receives
    -- a table and aborts the entire loadout before later weapons are given.
    if istable(equipment) then
        local result = true
        for _, equipment1 in pairs(equipment) do
            if hg.AddArmor(ply, equipment1) == false then result = false end
        end
        return result
    end
	if not isstring(equipment) then return false end
