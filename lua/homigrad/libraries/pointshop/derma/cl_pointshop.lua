...
local function createButton(k,ent,size,Pan,mainpan)
    ...
    timer.Simple(0.1,function()
        ...
        BPan:SetText( LocalPlayer():PS_HasItem(ent.ID) and "已售出" or ent.ISDONATE and "捐赠" or "购买："..ent.PRICE.." ZP" )
        ...
        function BPan:DoClick()
            ...
            if LocalPlayer():PS_HasItem(ent.ID) then 
                self:SetText( LocalPlayer():PS_HasItem(ent.ID) and "已售出" or "购买："..(ent.ISDONATE and "捐赠" or ent.PRICE.." ZP")  ) 
            return end
            self:SetText( "请稍候..." )
            self.InWait = true
...
//
    //    function SPan:Paint(w,h)
    //    end
    local tbl = col:AddSheet( "ZP 商店", SPan, "icon16/basket.png" )
    tbl["Button"]:SetFont("HomigradFontBig")
    tbl["Button"]:SizeToContents()
    tbl["Button"]:SetContentAlignment(6)
    -- Fill Panel End
