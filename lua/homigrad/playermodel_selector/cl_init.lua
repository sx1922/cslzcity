...
function Menu()
	...
	CloseCross(Category(content, "玩家模型选择器", true), frame)
	...
	local applyBtn = Btn(btnRow, "立即应用", clr.green, clr.greenhov)
	applyBtn.DoClick = function()
		surface.PlaySound(sndClick)
		SendApply()
	end

	local resetBtn = Btn(btnRow, "恢复外观", clr.red, clr.redhov)
	...
	nickPanel.Paint = function(self, w, h)
		...
		surface.SetDrawColor(clr.rowbar)
		surface.DrawRect(0, h - 3, w, 3)
		draw.SimpleText("外观名称", "ZB_PMS_Item", 12, 6, clr.text)
		draw.SimpleText("临时 - 死亡后恢复你的正常名字", "ZB_PMS_Small", w - 12, 12, clr.dim, TEXT_ALIGN_RIGHT)
	end
	...
	preview.PaintOver = function(self, w, h)
		surface.SetDrawColor(clr.border)
		surface.DrawOutlinedRect(0, 0, w, h, 1)
		draw.SimpleText("按住鼠标左键旋转", "ZB_PMS_Small", w / 2, h - 10, clr.dim, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
	end
	...
	rebuildCustomize = function()
		...
		if ent:SkinCount() > 1 then
			hasAny = true
			ArrowRow("皮肤", function() return ent:SkinCount() end, function() return sel.skin end, function(v)
		...
		for _, bg in ipairs(ent:GetBodyGroups() or {}) do
			...
			hasAny = true
			ArrowRow(bg.name or ("身体组 " .. id), function() return count end, function() return sel.groups[id] or 0 end, function(v)
				sel.groups[id] = v
				ApplyToPreview()
				QueueApply()
			end)
