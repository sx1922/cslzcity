			...
			if mouseClick == 1 then
				...
				for i, str in ipairs(randomGestures) do
					commands[i] = {
						...
						[2] = gestureNames[istable(str) and str[1] or str] or string.NiceName(istable(str) and str[1] or str)
					}
				end
...
if game.SinglePlayer() then
	hook.Add("HUDPaint","Exit the singleplayer",function()
		draw.SimpleText("Z-City不适合在单人模式下运行，在地图选择菜单中将单人模式(右上角绿色按钮)改为2人模式或更多。", "HomigradFontMedium", ScrW() / 2,ScrH() / 2, nil, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText("许多功能将无法正常运行，我们永远不会为单人模式提供任何修复。", "HomigradFontMedium", ScrW() / 2,ScrH() * 7 / 12, nil,TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end)
end
