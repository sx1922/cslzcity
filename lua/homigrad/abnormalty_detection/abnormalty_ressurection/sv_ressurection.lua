...
local function Ressurect(owner, body, time)
	PLUGIN.ShowMessageToAll("有什么事情正在发生")
	...
	if(PLUGIN.GetZoneOrPlyBlood(zone, ply) >= blood_consumption)then
		...
		if(body)then
			...
			if(!PLUGIN.Ressurection.ToRessurect[owner])then
				PLUGIN.ShowMessageInSphere("正在复活 " .. body:GetNWString("PlayerName") .. "...", zone.Pos, zone.Radius)
				...
				PLUGIN.ShowMessage(ply, "区域内没有尸体")
			end
			-- PLUGIN.AddConsequences(ply, 200)
		else
			PLUGIN.ShowMessage(ply, "该区域内没有尸体")
		end
	else
		PLUGIN.ShowMessage(ply, "血液不足，无法复生尸体")
	end
end
--//
	...
	for ply, info in pairs(PLUGIN.Ressurection.ToRessurect) do
		if(info.Time <= CurTime())then
			...
			if(!IsValid(owner) or owner:Alive() or !IsValid(body))then
				PLUGIN.ShowMessageToAll("仪式被神圣干预打断了。\n腐化正在蔓延")
			else
				...
				PLUGIN.ShowMessageToAll("有邪恶的事情发生了")
			end
