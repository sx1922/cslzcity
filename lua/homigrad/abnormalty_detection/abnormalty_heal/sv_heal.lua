...
local function TryHeal(zone, ply)
	...
	if(PLUGIN.GetZoneOrPlyBlood(zone, ply) >= blood_consumption)then
		...
		if(owner and !PLUGIN.Heal.ToHeal[owner])then
			Heal(owner, 5)
			PLUGIN.ShowMessageInSphere("正在治疗 " .. owner:GetNWString("PlayerName") .. "...", zone.Pos, zone.Radius)
		...
		PLUGIN.ShowMessage(ply, "血液不足，无法治疗")
	end
end
--//
	...
	for ply, info in pairs(PLUGIN.Heal.ToHeal) do
		if(info.Time <= CurTime())then
			...
			if(IsValid(owner) and owner:Alive())then
				...
				if(zone)then
					PLUGIN.ShowMessageInSphere("已治疗 " .. owner:GetNWString("PlayerName"), zone.Pos, zone.Radius)
				end
			else
