		...
		if(ply.AbnormaltiesReady)then
			...
			if(!knowledge["consequences"])then
				if(amt <= -20 or amt >= 20)then
					...
					PLUGIN.ShowMessage(ply, "你现在可以阅读第8页")
				end
			end
			...
			if(!knowledge["instabillity"])then
				if(amt <= -300 or amt >= 300)then
					...
					PLUGIN.ShowMessage(ply, "你现在可以阅读第9页")
				end
			end
			...
			if(!knowledge["positive_instabillity"])then
				if(amt >= 300)then
					...
					PLUGIN.ShowMessage(ply, "你现在可以阅读第13页")
					PLUGIN.ShowMessage(ply, "你现在可以阅读第15页")
				end
			end
			...
			if(!knowledge["negative_instabillity"])then
				if(amt <= -300)then
					...
					PLUGIN.ShowMessage(ply, "你现在可以阅读第14页")
					PLUGIN.ShowMessage(ply, "你现在可以阅读第16页")
				end
			end
			...
			if(!knowledge["insanity"])then
				if(amt <= -500 or amt >= 500)then
					...
					PLUGIN.ShowMessage(ply, "你现在可以阅读第10页")
				end
			end
					...
					local found_zone = PLUGIN.DoWithinZones(pos, function(zone_id, zone)
						...
						if(zone.Points > PLUGIN.HotZonePoints)then
							if(!PLUGIN.HotZones[zone_id])then
								...
								PLUGIN.ShowMessageInSphere("区域已扩张到足以开始积累词句与进行仪式", zone.Pos, zone.Radius)
							end
...
--\\??
	...
	区域点数: ]] .. math.Round(zone.Points or 0) .. [[

	区域半径: ]] .. math.Round(zone.Radius or 0) .. [[
	
	距中心距离: ]] .. math.Round(ply:GetPos():Distance(zone.Pos)) .. [[
	
	区域血液: ]] .. math.Round(zone.Blood or 0) .. [[
								]])
								end)
								
								if(!found_zone)then
									PLUGIN.ShowMessage(ply, "未找到任何区域")
								end
							else
								PLUGIN.ShowMessage(ply, "你的话语似乎消散于虚无之中")
							end
						else
							PLUGIN.LoadConsequences(ply)
						end
	...
	平衡度: ]] .. math.Round(PLUGIN.GetConsequences(ply) or 0) .. [[

	异常血液: ]] .. math.Round(ply.Abnormalties_Blood or 0) .. [[

	异常均衡器: ]] .. math.Round(ply.Abnormalties_Equalizers or 0) .. [[
								]])
							else
								PLUGIN.ShowMessage(ply, "你的话语似乎消散于虚无之中")
							end
					...
					if(!ply.AbnormaltiesNextFindPhrase or ply.AbnormaltiesNextFindPhrase <= CurTime())then
						if(ply.AbnormaltiesReady)then
							...
							if(knowledge["insanity"])then
								...
								PLUGIN.ShowMessage(ply, "某个声音低语着:")
				...
				if(ply.Abnormalties_RecentPhraseAmt == ply.Abnormalties_PhraseAmtToShow)then
					if(uselessness == 0)then
						...
						PLUGIN.ShowMessage(ply, "这些字母没有任何含义……")
					end
				end
			end
		end
