...
function PANEL:Paint(w, h)
	...
	DisableClipping(true)
		draw.SimpleText("按住左ALT并按下回车以耳语", "zChatFontSmall", 5, h * 1.01 + 1, black)
		draw.SimpleText("按住左ALT并按下回车以耳语", "zChatFontSmall", 4, h * 1.01, gray)

		if LocalPlayer().organism and LocalPlayer().organism.otrub  then
			draw.SimpleText("你的消息目前对任何人都不可见。", "zChatFontSmall", ScrW() * 0.3 + 1, h * 1.01 + 1, black, TEXT_ALIGN_RIGHT)
			draw.SimpleText("你的消息目前对任何人都不可见。", "zChatFontSmall", ScrW() * 0.3, h * 1.01, gray, TEXT_ALIGN_RIGHT)
		end
	DisableClipping(false)
