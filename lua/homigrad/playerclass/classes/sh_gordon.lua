...
if CLIENT then
    ...
    function CLASS.HUDPaint(self)
        ...
        draw.DrawText("医疗剂","HEVFontSmall",pos[1]+size[1]*0.085+1,pos[2]+(size[2]/1.8)+1,color_bg,TEXT_ALIGN_LEFT)
        draw.DrawText("医疗剂","HEVFontSmall",pos[1]+size[1]*0.085,pos[2]+(size[2]/1.8),color_hp,TEXT_ALIGN_LEFT)
        ...
        draw.DrawText("血液/Ml","HEVFontSmall",pos[1]+size[1]*0.085+1,pos[2]+(size[2]/2)+1,color_bg,TEXT_ALIGN_LEFT)
        draw.DrawText("血液/Ml","HEVFontSmall",pos[1]+size[1]*0.085,pos[2]+(size[2]/2),color_bld,TEXT_ALIGN_LEFT)
    end
    ...
    hook.Add("Org Think","gordon_healing",function(ply, org, timeValue)
        ...
        if ply.PlayerClassName == "Gordon" and ply:GetNetVar("HEVSuit") then
            ...
            if org.brain > 0.1 then
                ...
                ply:Notify("HEV防护服检测到创伤性脑损伤。正在注射甘露醇。",true,"mannitol_hev",0.5,function(ply)
            ...
            if org.pneumothorax > 0 then
                ...
                ply:Notify("HEV防护服检测到气胸。正在修复。", true, "needle_hev", 0.5, function(ply)
                    net.Start("HEV_DAMAGE")
                        net.WriteString("hl1/fvox/automedic_on.wav")
                    net.Send(ply)
                end, hev_color)
            ...
            if pain > 30 then
                if (org.NextMorphineInject or 0) < CurTime() then
                    ...
                    if administer > 0.1 then
                        ply:Notify("HEV防护服检测到痛觉感受器即将达到阈值。正在注射吗啡。", 10, "morphine_hev", 0.5,
                        function(ply)
                            net.Start("HEV_DAMAGE")
            ...
            if (org.CO > 10) or (org.COregen > 10) then
                ply:Notify("HEV防护服检测到体内存在一氧化碳。正在中和。",60,"co_hev",0.5,function(ply)
                    net.Start("HEV_DAMAGE")
                        net.WriteString("hl1/fvox/automedic_on.wav")
                    net.Send(ply)
                end, hev_color)
            ...
            if (org.BoneCheck or 0) < CurTime() then
                ...
                if bonesfixed then
                    ply:Notify("HEV防护服检测到骨折。正在修复。",60,"bones_hev",0.5,function(ply)
                        net.Start("HEV_DAMAGE")
                            net.WriteString("hl1/fvox/automedic_on.wav")
                        net.Send(ply)
                    end, hev_color)
            ...
            if (org.pulse < 40) or (org.blood < 3000) or (org.o2[1] < 10) then
                ply:Notify("HEV防护服检测到脉搏极低。已注射肾上腺素。自动脉搏已启用。已注射血浆。", 60, "pulse_hev", 0.5, function(ply)
                    net.Start("HEV_DAMAGE")
