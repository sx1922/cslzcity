...
SWEP.PrintName = "氰化物罐"
SWEP.Instructions = "产生气体,阻止电子从细胞色素c传递到氧气。结果,电子传递链被破坏,意味着细胞无法再通过有氧呼吸产生ATP作为能量。高度依赖有氧呼吸的组织,如中枢神经系统和心脏,特别受到影响。"
...
if SERVER then
	...
	hook.Add("Org Think", "poison3",function(owner, org, timeValue)
		...
		if ((org.poison3 + 4) < CurTime()) and owner.Profession == "cook" then
			org.owner:Notify("这里有股杏仁味...是香水吗？", true, "cyanide", 3)
		end

		if (not org.poison3notificate) and ((org.poison3 + 20) < CurTime()) then
			org.poison3notificate = true
			org.owner:Notify("呼吸变得越来越困难...由于某种原因...", true, "cyanide2", 3)
