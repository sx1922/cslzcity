...
SWEP.PrintName = "对讲机"
SWEP.Instructions = "使用对讲机与4公里范围内的其他人通讯。必须在相同频率上。"
	...
	if(IsValid(owner))then
		...
		cam.Start3D2D( newPos, newAng, 0.005 )
			...
			if self:GetIsOn() then
				...
				draw.SimpleText(self:GetOwner():IsSpeaking() and "发送中" or "接收中", "Walkie-Talkie_Fixed-SmallFont", 0, 40, walkietalkie_clr, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		cam.End3D2D()
...
if CLIENT then
	function SWEP:MenuAddAdjuster(strName, tbl, howmuch)
		tbl[#tbl + 1] = {function()
			local tbl1 = {}
			tbl1[#tbl1 + 1] = {function() RunConsoleCommand("hg_walkietalkie_adjust", howmuch) return -1 end,"增大"}
			tbl1[#tbl1 + 1] = {function() RunConsoleCommand("hg_walkietalkie_adjust", -howmuch) return -1 end,"减小"}
	...
	if self:GetIsOn() then
		tbl[#tbl + 1] = {function()
			...
			for i = 1, #self.Frequencies do
				...
				tbl1[#tbl1 + 1] = { function() RunConsoleCommand("hg_walkietalkie_adjust", station - self:GetHudFrequency() ) end, "电台 " .. station .. "MHz" }
				hg.CreateRadialMenu(tbl1)
			end
			return -1
		end, "公共电台"}
