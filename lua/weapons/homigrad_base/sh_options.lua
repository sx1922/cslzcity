if CLIENT then
	...
	concommand.Add("hg_change_ammotype", function(ply, cmd, args)
	    ...
	    if wep and ishgweapon(wep) and (wep:Clip1() == 0 or wep.AllwaysChangeAmmo) and wep:CanUse() and wep.AmmoTypes and wep.AmmoTypes[type_] then
	        ply:ChatPrint("切换弹药类型为: " .. wep.AmmoTypes[type_][1])
...
hg.postures = {
    ...
    [5] = "指向射击",
    [6] = "掩体射击",
    [7] = {"帮派式持枪",isPistolOnly = true},
    [8] = {"单手",isPistolOnly = true},
	[9] = "索马里式",
}
...
if CLIENT then
	...
	concommand.Add("hg_change_posture", function(ply, cmd, args)
		if not args[1] and not isnumber(args[1]) and not printed then print([[更改你的持枪姿态：
0 - 常规持枪
1 - 腰射
2 - 左肩持枪
3 - 高预备位
