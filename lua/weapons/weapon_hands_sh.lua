...
SWEP.Category = "ZCity 其他"
SWEP.Instructions = "左键 - 举起拳头\nR键 - 放下拳头\n\n举起状态下：\n左键 - 攻击\n右键 - 格挡\n\n放下状态下：右键 - 抬起物体，右键+R - 检查脉搏（对准他人的头部或手部使用）\n\n持有物体时：R键 - 将物体固定在空中，E键 - 在空中旋转物体。"
SWEP.Spawnable = true
...
if SERVER then
	...
	function SWEP:DrawHUD()
		...
		if ent.organism then
			...
			local txt = "正在显示 "..ent:GetPlayerName().." 的伤势："
			...
			if org.bleed and org.bleed > 0.1 then
				hg.DrawAffliction(posx + add_x, posy - h, w, h, math.min(org.bleed / 10, 1), hg.afflictions.bleeding, lerpalpha, "流血")
			...
			if org.rleg and org.rleg > 0 then
				hg.DrawAffliction(posx + add_x, posy - h, w, h, org.rleg, org.rleg > 0.999 and hg.afflictions.lfracture or hg.afflictions.lblunt, lerpalpha, org.rleg > 0.999 and "右腿骨折" or "右腿钝器伤")

				add_x = add_x + w + add
			end

			if org.lleg and org.lleg > 0 then
				hg.DrawAffliction(posx + add_x, posy - h, w, h, org.lleg, org.lleg > 0.999 and hg.afflictions.lfracture or hg.afflictions.lblunt, lerpalpha, org.lleg > 0.999 and "左腿骨折" or "左腿钝器伤")

				add_x = add_x + w + add
			end

			if org.rarm and org.rarm > 0 then
				hg.DrawAffliction(posx + add_x, posy - h, w, h, org.rarm, org.rarm > 0.999 and hg.afflictions.afracture or hg.afflictions.ablunt, lerpalpha, org.rarm > 0.999 and "右臂骨折" or "右臂钝器伤")

				add_x = add_x + w + add
			end

			if org.larm and org.larm > 0 then
				hg.DrawAffliction(posx + add_x, posy - h, w, h, org.larm, org.larm > 0.999 and hg.afflictions.afracture or hg.afflictions.ablunt, lerpalpha, org.larm > 0.999 and "左臂骨折" or "左臂钝器伤")
			...
			if org.o2 and org.o2[1] < 5 then
				hg.DrawAffliction(posx + add_x, posy - h, w, h, (5 - org.o2[1]) / 5, hg.afflictions.lung_failure, lerpalpha, "肺部衰竭")
				...
				if not ply2.noHead and ply2.organism then
					...
					elseif ((bone == "ValveBiped.Bip01_L_Hand") or (bone == "ValveBiped.Bip01_R_Hand") or (bone == "ValveBiped.Bip01_Head1")) then
						...
						if org.heartstop then
							...
							ply:ChatPrint(org.pulse < 20 and "几乎感觉不到脉搏。" or (org.pulse <= 50 and "脉搏偏低。") or (org.pulse <= 90 and "脉搏正常。") or "脉搏偏高。")
						end

						if (org.last_heartbeat + 60) > CurTime() then
							ply:ChatPrint("尸体仍然温热。")
						else
							ply:ChatPrint((org.last_heartbeat + 180) < CurTime() and "尸体已经在这里有一段时间了。" or "尸体还微微发热")
						end
						...
						if org.bleed > 0 then
							ply:ChatPrint("尸体正在流血，"..((org.bleed > 10 and "血流不止。") or (org.bleed > 5 and "流血适中。") or "轻微流血。"))
						end
						...
						if org.bulletwounds > 0 then
							ply:ChatPrint("你发现这具尸体上有 "..org.bulletwounds.." 处枪伤。")
						end

						if org.stabwounds > 0 then
							ply:ChatPrint("你发现这具尸体上有 "..org.stabwounds.." 处刺伤。")//28 STAB WOUNDS. YOU WOULDNT LEAVE HIM A CHANCE, HUH?
						end

						if org.slashwounds > 0 then
							ply:ChatPrint("你发现这具尸体上有 "..org.slashwounds.." 处划伤。")
						end

						if org.bruises > 0 then
							ply:ChatPrint("你发现这具尸体上有 "..org.bruises.." 处淤伤。")
						end

						if org.burns > 0 then
							ply:ChatPrint("这具尸体曾被焚烧过。")
						end

						if org.explosionwounds > 0 then
							ply:ChatPrint("这具尸体看起来有爆炸造成的创伤。")
						end
				...
				if org and bone == "ValveBiped.Bip01_Spine2" and trace.Hit then
					if self.firstTimePrint then
						if not ply2.noHead then
							ply:ChatPrint("你开始进行心肺复苏。")
						else
							ply:Notify("我觉得心肺复苏在这里没帮助...", 10)
						end
					end
...
local customClassInfo = {
	...
	["furry"] = {
		...
		handsDesc = "furry",
		Instructions = "左键 - 举起利爪\nR键 - 放下利爪\n\n举起状态下：\n左键 - 攻击\n右键 - 格挡\n\n<color=91,121,229>作为pathowogen感染的携带者，你获得了新的能力。\n\n放下状态下，按住右键抓住未感染的猎物，然后按住左键同化他们。\n\n你可以按左键舔舐你的同伴，这有助于缓解他们的疼痛。\n\n:3<color=180,180,180>"
	},
	["headcrabzombie"] = {
		PrintName = "利爪",
		WepSelectIcon = Material("vgui/wep_jack_hmcd_zombhands"),
		handsDesc = "zombie",
		Instructions = "左键 - 攻击\n右键 - 格挡/抓住玩家\n\n<color=150,0,0>这是你被僵尸化的双手。它们虽不是光剑，但依然威力十足。"
	}
}
...
function SWEP:PrimaryAttack(forcespecial)
	...
	self:SetNextPrimaryFire(CurTime() + .35 * math.Clamp((180 - (owner.organism and owner.organism.stamina[1] or 180)) / 90,1,2) + (math.max(special_attack and 0.5 or 0, clawClasses[owner.PlayerClassName] or 0)))
