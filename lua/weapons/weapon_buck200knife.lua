...
SWEP.PrintName = "巴克120通用刀" -- 巴克120通用刀
SWEP.Instructions = "大型猎刀，带有血槽，可以进行造成强出血的刺击。在电影《惊声尖叫》中用作杀手的主要武器。\n\n左键攻击。\nR + 左键改变攻击模式。\n右键格挡。"
SWEP.Category = "武器 - 近战"
SWEP.Spawnable = true
...
function SWEP:Reload()
    if SERVER then
        if self:GetOwner():KeyPressed(IN_ATTACK) then
            ...
            self:GetOwner():ChatPrint("已切换至"..(self:GetNetVar("mode") and "劈砍模式。" or "突刺模式。"))
        end
    end
end
