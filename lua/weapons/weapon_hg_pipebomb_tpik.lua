...
SWEP.PrintName = "管道炸弹" -- 管道炸弹
SWEP.Instructions =
[[这种简易爆炸装置是一个填充黑火药并周围包裹钉子的重型钢管。它有一个简单的短烟火引信。该装置通过对黑火药（一种低爆炸药）的紧密封装实现高爆速和破片投射。不过它仍然不如正规手榴弹那样致命或可靠。
...
SWEP.WorldModelExchange = "models/w_models/weapons/w_jj_pipebomb.mdl"
SWEP.basebone = 57
SWEP.HideMeshBones = {
	"Weapon",
	"Liq_base",
...
SWEP.weaponPos = Vector(0.8,-0.5,-1.5)
SWEP.weaponAng = Angle(0,140,0)
SWEP.modelscale = 1.1

SWEP.lefthandmodel = "models/weapons/gleb/w_firematch.mdl"
SWEP.offsetVec2 = Vector(3.1,-0.8,1.5)
SWEP.offsetAng2 = Angle(10,0,80)
...
function SWEP:DrawPostPostModel()
	if self.Burn and (!self.sparkThink or self.sparkThink < CurTime()) then
		local Spark = EffectData()
		local att = self:GetWM():GetAttachment(2)
		Spark:SetOrigin(att.Pos)
		Spark:SetScale(2)
