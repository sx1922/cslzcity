...
SWEP.PrintName = "河豚毒素注射器"
SWEP.Instructions = "河豚毒素是一种强效毒药,由日本科学家于1906年发现。死亡由呼吸肌麻痹引起。只能注射到脊神经中。"
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		if IsValid(ply) and (ply:IsPlayer() or ply:IsRagdoll()) and caninjectbone[ply:GetBoneName(ply:TranslatePhysBoneToBone(tr.PhysicsBone))] and self:CanInject(ply,tr.PhysicsBone) then
			draw.SimpleText( "注射", "HomigradFont", toScreen.x + 3, toScreen.y + 27, color_black, TEXT_ALIGN_CENTER )
			draw.SimpleText( "注射", "HomigradFont", toScreen.x, toScreen.y + 25, colred, TEXT_ALIGN_CENTER )
...
if SERVER then
	...
	hook.Add("Org Think", "poison1",function(owner, org, timeValue)
		...
		if (not org.poison1notificate) and ((org.poison1 + 20) < curtime) then
			...
			org.owner:Notify("我无法...正常呼吸...", true, "poison1", 3)
