...
SWEP.Instructions = "装在小纸板盒里的自制C4炸药。起爆器是一部旧的诺基亚手机。将炸弹放入不同物体中以产生破片或火焰。左键放入物体,右键直接放置炸弹。放置后左键激活。"
SWEP.Category = "ZCity 爆炸武器"
SWEP.Spawnable = true
	...
	function SWEP:DrawHUD()
		...
		if IsValid(tr.Entity) and not tr.Entity:IsPlayer() and not tr.Entity:IsRagdoll() and not self:GetPlanted() then
			...
			if tr.MatType == MAT_METAL then
				draw.SimpleText( "它爆炸时会迸出弹片。", "HomigradFont", toScreen.x+3, toScreen.y + 25 + 32, color_black, TEXT_ALIGN_CENTER )
				draw.SimpleText( "它爆炸时会迸出弹片。", "HomigradFont", toScreen.x, toScreen.y + 25 + 30, colblue, TEXT_ALIGN_CENTER )
				end

				if FireEnts[tr.Entity:GetModel()] then
				draw.SimpleText( "它爆炸时会引发火灾。", "HomigradFont", toScreen.x+3, toScreen.y + 25 + 62, color_black, TEXT_ALIGN_CENTER )
				draw.SimpleText( "它爆炸时会引发火灾。", "HomigradFont", toScreen.x, toScreen.y + 25 + 60, colred, TEXT_ALIGN_CENTER )
				end

				draw.SimpleText( "可安放于物体上。", "HomigradFont", toScreen.x + 3, toScreen.y + 27, color_black, TEXT_ALIGN_CENTER )
				draw.SimpleText( "可安放于物体上。", "HomigradFont", toScreen.x, toScreen.y + 25, color_white, TEXT_ALIGN_CENTER )
				elseif self:GetPlanted() then		
				local xrand,yrand = math.random(-1,1),math.random(-1,1)
				draw.SimpleText( "左键引爆。", "HomigradFontMedium", toScreen.x + 2 + xrand, toScreen.y + 26 + yrand, color_black, TEXT_ALIGN_CENTER )
				draw.SimpleText( "左键引爆。", "HomigradFontMedium", toScreen.x + xrand, toScreen.y + 25 + yrand, color_red, TEXT_ALIGN_CENTER )
		end
	end
end
