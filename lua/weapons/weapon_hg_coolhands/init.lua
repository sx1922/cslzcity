				...
				if not ply2.noHead and ply2.organism then
					...
					elseif ((bone == "ValveBiped.Bip01_L_Hand") or (bone == "ValveBiped.Bip01_R_Hand") or (bone == "ValveBiped.Bip01_Head1")) then
						...
						if (org.last_heartbeat + 60) > CurTime() then
							ply:ChatPrint("尸体仍然温暖。")
						else
							ply:ChatPrint((org.last_heartbeat + 180) < CurTime() and "这具尸体已经在这里有一段时间了。" or "尸体还略带温度。")
						end
						...
						if org.bleed > 0 then
							ply:ChatPrint("尸体正在流血"..((org.bleed > 10 and "，流血严重。") or (org.bleed > 5 and "，流血中等。") or "，流血轻微。"))
						end
						...
						if org.bulletwounds > 0 then
							ply:ChatPrint("你注意到这具尸体上有 "..org.bulletwounds.." 处枪伤。")
						end

						if org.stabwounds > 0 then
							ply:ChatPrint("你注意到这具尸体上有 "..org.stabwounds.." 处刺伤。")//28 STAB WOUNDS. YOU WOULDNT LEAVE HIM A CHANCE, HUH?
						end

						if org.slashwounds > 0 then
							ply:ChatPrint("你注意到这具尸体上有 "..org.slashwounds.." 处割伤。")
						end

						if org.bruises > 0 then
							ply:ChatPrint("你注意到这具尸体上有 "..org.bruises.." 处淤伤。")
						end

						if org.burns > 0 then
							ply:ChatPrint("这具尸体曾被烧过。")
						end

						if org.explosionwounds > 0 then
							ply:ChatPrint("这具尸体似乎受到了爆炸创伤。")
						end
