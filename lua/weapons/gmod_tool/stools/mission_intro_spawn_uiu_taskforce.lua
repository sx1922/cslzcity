...
TOOL.Name = "特异事故处(FBI)出生点"
TOOL.Command = nil
TOOL.Information = {
	{ name = "left", text = "放置特异事故处(FBI)出生点" },
...
local MI_FACTION = "uiu_taskforce"
...
function TOOL.BuildCPanel(panel)
	panel:Help("左键：放置特异事故处 FBI 编队专用入场出生点")
	panel:Help("右键：删除瞄准的出生点")
	panel:Help("重装：清除地图上全部出生点")
	panel:NumSlider("朝向偏移 (Yaw)", "mission_intro_spawn_uiu_taskforce_yaw", -180, 180, 0)
	panel:Help("阵营 ID：uiu_taskforce（与旧 cultist 特工组 sid_squad 独立）")
end

if CLIENT then
	language.Add("tool.mission_intro_spawn_uiu_taskforce.name", "特异事故处(FBI)出生点")
	language.Add("tool.mission_intro_spawn_uiu_taskforce.desc", "放置特异事故处 FBI 编队任务入场出生点")
	language.Add("tool.mission_intro_spawn_uiu_taskforce.left", "放置出生点")
	language.Add("tool.mission_intro_spawn_uiu_taskforce.right", "删除出生点")
	language.Add("tool.mission_intro_spawn_uiu_taskforce.reload", "清除全部")
end
