...
local MI_FACTION = "hammerfall_squad"
...
function TOOL.BuildCPanel(panel)
	panel:Help("左键：放置落锤作战分队专用入场出生点")
	panel:Help("右键：删除瞄准的出生点")
	panel:Help("重装：清除地图上全部出生点")
	panel:NumSlider("朝向偏移 (Yaw)", "mission_intro_spawn_hammerfall_yaw", -180, 180, 0)
	panel:Help("落锤入场玩家只会在此类出生点（或通用点）复活。")
end

if CLIENT then
	language.Add("tool.mission_intro_spawn_hammerfall.name", "落锤出生点")
	language.Add("tool.mission_intro_spawn_hammerfall.desc", "放置落锤作战分队任务入场出生点")
	language.Add("tool.mission_intro_spawn_hammerfall.left", "放置落锤出生点")
	language.Add("tool.mission_intro_spawn_hammerfall.right", "删除出生点")
	language.Add("tool.mission_intro_spawn_hammerfall.reload", "清除全部")
end
