...
TOOL.Command = nil
TOOL.Information = {
	{ name = "left", text = "放置混沌同盟撤离点（阵营0）" },
	{ name = "right", text = "删除瞄准的撤离点" },
	{ name = "reload", text = "清除全部混沌同盟撤离点" },
}

TOOL.ClientConVar["yaw"] = "0"
TOOL.ClientConVar["radius"] = "130"

local MI_BATTLE_TEAM = 0
...
function TOOL:RightClick(trace)
	...
	if IsValid(ent) and ent:GetClass() == "ent_mission_intro_rxsend_evac" and ent:GetBattleTeam() == MI_BATTLE_TEAM then
		if self:GetOwner():KeyDown(IN_SPEED) and ent.SetEvacZoneRadius then
			...
			self:GetOwner():ChatPrint("[RX] 已更新混沌同盟撤离点半径")
			return true
		end
...
function TOOL.BuildCPanel(panel)
	panel:Help("左键：放置混沌同盟（阵营0）撤离点")
	panel:Help("RXsend 回合最后 20 秒开放；区域内停留 0.1 秒自动撤离。")
	panel:Help("右键：删除；Shift+右键：更新半径；重装：清除全部混沌同盟撤离点")
	panel:NumSlider("方形半边长", "mission_intro_rxsend_evac_radius", 64, 400, 0)
	panel:NumSlider("朝向偏移 (Yaw)", "mission_intro_rxsend_evac_ci_yaw", -180, 180, 0)
end

if CLIENT then
	language.Add("tool.mission_intro_rxsend_evac_ci.name", "混沌同盟撤离点")
	language.Add("tool.mission_intro_rxsend_evac_ci.desc", "放置 RXsend 混沌同盟阵营撤离区域")
	language.Add("tool.mission_intro_rxsend_evac_ci.left", "放置撤离点")
	language.Add("tool.mission_intro_rxsend_evac_ci.right", "删除撤离点")
	language.Add("tool.mission_intro_rxsend_evac_ci.reload", "清除全部")
end
