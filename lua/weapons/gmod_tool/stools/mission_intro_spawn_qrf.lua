...
local MI_FACTION = "qrf_taskforce"
...
function TOOL.BuildCPanel(panel)
	panel:Help("左键：放置快速反应部队专用出生点")
	panel:Help("右键：删除瞄准的出生点")
	panel:Help("重装：清除地图上全部出生点")
	panel:NumSlider("朝向偏移 (Yaw)", "mission_intro_spawn_qrf_yaw", -180, 180, 0)
	panel:Help("快速反应部队入场玩家只会在此类出生点（或通用点）复活。")
end

if CLIENT then
	language.Add("tool.mission_intro_spawn_qrf.name", "快速反应部队出生点")
	language.Add("tool.mission_intro_spawn_qrf.desc", "放置快速反应部队设施入场出生点")
	language.Add("tool.mission_intro_spawn_qrf.left", "放置 QRF 出生点")
	language.Add("tool.mission_intro_spawn_qrf.right", "删除出生点")
	language.Add("tool.mission_intro_spawn_qrf.reload", "清除全部")
end
