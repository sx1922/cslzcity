...
function TOOL.BuildCPanel(panel)
	panel:Help("左键：放置混沌分裂者专用入场出生点")
	panel:Help("右键：删除瞄准的出生点")
	panel:Help("重装：清除地图上全部出生点")
	panel:NumSlider("朝向偏移 (Yaw)", "mission_intro_spawn_ci_yaw", -180, 180, 0)
	panel:Help("该阵营玩家会优先在此类出生点（或通用点）复活。")
	panel:Help("须切换到工具枪并选中本工具，才能看到地面齿轮标记（与 UIU/落锤相同）。")
	panel:Help("控制台 mission_intro_show_spawns 可强制显示标记。")
end

if CLIENT then
	language.Add("tool.mission_intro_spawn_ci.name", "混沌分裂者出生点")
	language.Add("tool.mission_intro_spawn_ci.desc", "放置混沌分裂者任务入场出生点")
	language.Add("tool.mission_intro_spawn_ci.left", "放置出生点")
	language.Add("tool.mission_intro_spawn_ci.right", "删除出生点")
	language.Add("tool.mission_intro_spawn_ci.reload", "清除全部")
end
