...
function TOOL.BuildCPanel(panel)
	panel:Help("左键：放置特异事故处特工专用入场出生点")
	panel:Help("右键：删除瞄准的出生点")
	panel:Help("重装：清除地图上全部出生点")
	panel:NumSlider("朝向偏移 (Yaw)", "mission_intro_spawn_sid_yaw", -180, 180, 0)
	panel:Help("特异事故处玩家会优先在此类出生点（或通用点）复活。")
end

if CLIENT then
	language.Add("tool.mission_intro_spawn_sid.name", "特异事故处出生点")
	language.Add("tool.mission_intro_spawn_sid.desc", "放置特异事故处特工任务入场出生点")
	language.Add("tool.mission_intro_spawn_sid.left", "放置特异事故处出生点")
	language.Add("tool.mission_intro_spawn_sid.right", "删除出生点")
	language.Add("tool.mission_intro_spawn_sid.reload", "清除全部")
end
