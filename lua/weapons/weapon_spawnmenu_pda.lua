...
SWEP.PrintName = "平板电脑" -- 平板电脑
SWEP.Instructions = ""
SWEP.Category = "武器 - 其他"
SWEP.Instructions = "只是一台平板电脑"
SWEP.Spawnable = true
...
if SERVER then
    ...
    net.Receive("Deliver",function( len, ply )
        ...
        for k, item in pairs(Cart) do
            if not item or not item[1] then wep:AddNotificate("不行。") return end
            ...
            if not entTbl or not CategoresAllowed[entTbl.Category] then wep:AddNotificate("不行。") return end
            if not item[1] or BlackList[item[1]] then wep:AddNotificate("不行。") return end
        end

        if CartWeight > 140 then wep:AddNotificate("重量过重，无法运送。") return end

        local Time = (CartWeight/KgInTime)*60
        if Time == 0 then wep:AddNotificate("请先把物品放入购物车。") return end
        ...
        if tr.HitSky then
            wep:AddNotificate("您的订单正在配货中，请稍候。")
            timer.Create(ply:EntIndex().."_Deliver",Time,1,function()
                wep:AddNotificate("您的包裹已送达。您有5分钟时间领取物品。")
            ...
            wep:AddNotificate("您必须到室外，我们才能为您送货。")
        end
...
Cost: Free]])
    ...
    local CategoryNames = {
        ...
        ["Explosive"] = "爆炸物",
        ["Medicine"] = "医疗",
        ["ZCity Other"] = "Z-City·其他",
        ["ZCity Ammo"] = "Z-City·弹药",
        ["ZCity Armor"] = "Z-City·护甲",
        ["ZCity Melee"] = "Z-City·近战",
        ["Other"] = "其他",
        ["Weapons"] = "武器",
    }
        ...
        for k,Category in pairs(Categores) do
            ...
            local label = string.StartsWith(k, "Weapons") and string.sub(k,11) or k
            sheetDeliver:AddSheet( CategoryNames[label] or label, Shop )
        end

    sheet:AddSheet( "商店", sheetDeliver )
