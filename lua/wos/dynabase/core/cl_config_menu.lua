...
function wOS.DynaBase:OpenConfigMenu()
	...
	for order, data in ipairs( order_checker ) do
		...
		validated.UpdateCast = function( pan )
			if !frame.Toggled then
				...
				icon:SetTooltip( "该项目当前已禁用"  )
			else
				if data.ServerValid then
					pan:SetImage( "icon16/accept.png" )
					icon:SetTooltip( "该项目已由服务器和客户端注册"  )
				else
					pan:SetImage( "icon16/error.png" )		
					icon:SetTooltip( "该项目仅由客户端注册，可能导致不可预知的问题"  )	
				end
			end
		end
		...
		if data.Type == WOS_DYNABASE.REANIMATION then
			...
			thetype:SetTooltip( "该附加内容会替换现有动画" )	
		else
			thetype:SetImage( extend_mat )
			thetype:SetTooltip( "该附加内容包含新的动画" )	
		end
		...
		if !check1 and !check2 then
			...
			if frame.Toggled then
				toggle:SetText( "卸载附加内容" )
			else
				toggle:SetText( "挂载附加内容" )
			end
	...
	local function PopulateEntries()
		...
		for order, data in ipairs( local_order_checker ) do
			...
			validated.UpdateCast = function( pan )
				if !frame.Toggled then
					pan:SetImage( "icon16/delete.png" )
					pan:SetTooltip( "该项目当前已禁用"  )
				else
					pan:SetImage( "icon16/accept.png" )
					pan:SetTooltip( "该项目当前已启用"  )
	...
	download_butt:SetText( "将服务器挂载转换为用户挂载（将覆盖同名的挂载！）" )
	download_butt.DoClick = function(pan) 
		...
		PopulateEntries()
		chat.AddText( color_white, "[", Color( 0, 175, 255 ), "wOS-DynaBase", color_white, "] 所有已注册的服务器挂载已添加到您的用户挂载列表！" )
	end
	...
	savebutt.DoClick = function( pan )
		if sheet:GetActiveTab():GetText():find( "用户" ) then
